...
function mockField() {
	return {
		as: (t: string) => ({ name: 'province', type: t }),
		issues: () => undefined,
		value: () => '',
		set: vi.fn(),
		touched: () => false,
...
test('renders a state <select> when country is configured (us)', async () => {
	await render(InputProvince, { field: mockField(), country: 'us' })
...
test('renders a freeform text input when country is unconfigured', async () => {
	await render(InputProvince, { field: mockField(), country: 'gb' })
...
test('custom config with military options renders them', async () => {
	...
	await render(InputProvince, { field: mockField(), country: 'us', config })
