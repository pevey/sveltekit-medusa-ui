...
describe('resolveQuantity', () => {
	it('prefers explicit prop', () => {
		expect(resolveQuantity(3, ctx({ quantity: 2 }))).toBe(3)
	})
	it('falls back to context', () => {
		expect(resolveQuantity(undefined, ctx({ quantity: 2 }))).toBe(2)
	})
	it('defaults to 1', () => {
		expect(resolveQuantity(undefined, null)).toBe(1)
	})
})
...
describe('cartSatisfiesCondition', () => {
	const c = cart([
		{
			...
			product_id: 'p1',
			product_handle: 'tee',
			product_collection: 'Warby Parker',
			quantity: 2
		}
	])
	it('variantId match', () => {
		expect(cartSatisfiesCondition(c, { variantId: 'v1' })).toBe(true)
	})
	it('productId match', () => {
		expect(cartSatisfiesCondition(c, { productId: 'p1' })).toBe(true)
	})
	it('productSlug match', () => {
		expect(cartSatisfiesCondition(c, { productSlug: 'tee' })).toBe(true)
	})
	...
	it('no match → false', () => {
		expect(cartSatisfiesCondition(c, { variantId: 'zzz' })).toBe(false)
	})
	it('empty condition → false', () => {
		expect(cartSatisfiesCondition(c, {})).toBe(false)
	})
})
