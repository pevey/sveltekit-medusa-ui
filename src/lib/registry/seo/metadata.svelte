<script lang="ts">
	...
	const title = $derived(!isHome && site.titleTemplate && baseTitle ? site.titleTemplate.replace('%s', baseTitle) : baseTitle)
	const description = $derived(config.description ?? site.description)
	const url = $derived(config.url ?? (site.siteUrl ? site.siteUrl.replace(/\/$/, '') + page.url.pathname : page.url.href))
