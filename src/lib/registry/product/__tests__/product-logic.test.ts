...
function fixture(): StoreProduct {
	const variants: Partial<StoreProductVariant>[] = [
		{
			id: 'v_rs',
			options: [OV('red', 'red', 'oColor'), OV('s', 'S', 'oSize')],
			manage_inventory: true,
			inventory_quantity: 5
		},
		{
			id: 'v_rm',
			options: [OV('red', 'red', 'oColor'), OV('m', 'M', 'oSize')],
			manage_inventory: true,
			inventory_quantity: 0
		},
		{
			id: 'v_bs',
			options: [OV('blue', 'blue', 'oColor'), OV('s', 'S', 'oSize')],
			manage_inventory: false,
			inventory_quantity: null
		},
		{
			id: 'v_bm',
			options: [OV('blue', 'blue', 'oColor'), OV('m', 'M', 'oSize')],
			manage_inventory: true,
			inventory_quantity: null
		}
	]
...
describe('isPurchasable', () => {
	it('null variant is not purchasable', () => {
...
describe('effectiveMax', () => {
	...
	it('treats manage_inventory null as managed (caps to stock)', () => {
		expect(
			effectiveMax(
				v({
					manage_inventory: null as unknown as boolean,
...
describe('clampQuantity', () => {
	it('clamps into range', () => {
		expect(clampQuantity(9, 1, 3)).toBe(3)
	})
	it('floors at min', () => {
		expect(clampQuantity(0, 1, 3)).toBe(1)
	})
	it('returns min when hi < min (empty range)', () => {
